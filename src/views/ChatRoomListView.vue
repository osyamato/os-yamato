...
<transition name="modal">
  ...
  >
    <div>
      <h3 class="modal-title">受信したチャット申請</h3>
...
.header-button {
  background-color: #f0f0f0; /* ライトモード用 */
  border: none;
  border-radius: 50%;
  width: 30px;  /* 小さく調整 */
  height: 30px;
...
.message-text {
  display: block;               /* ✅ inline → block に変更 */
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
  flex-grow: 1;
  flex-shrink: 1;
  flex-basis: 0;
  min-width: 0;                 /* ✅ これが無いと flex-item の省略が効かない */
  max-width: 100%;
}
...
.blink {
  animation: blink-animation 2.4s infinite; /* ← ゆっくり */
}
...
.header-title {
  ...
  font-weight: bold;
  font-family: 'serif';   /* 和風寄りな雰囲気を演出（任意） */
  color: #274c77;         /* 和風な淡い青 */
  text-align: center;
}
...
.modal-title {
  font-size: 1.5rem;
  margin-bottom: 2rem;
  color: inherit; /* ← これで親の .request-card の色を継承 */
}
...
.request-card {
  background: #ffffff;         /* ライトモード用 背景 */
  color: #111111;              /* ライトモード用 テキスト色（←重要） */
  border-radius: 12px;
  padding: 1.5rem;
  max-width: 520px;
  margin: auto;
...
@media (prefers-color-scheme: dark) {
  .request-card {
    background: #1f1f1f;       /* ダークモード用 背景 */
    color: #f5f5f5;            /* ダークモード用 テキスト色 */
